...
export default function HomePage() {
  ...
  /* ── Auto-set platform filter based on device on first load ── */
  // We use the manual platformFilter; "all" means auto-detect
  ...
  const filtered = useMemo(() => {
    ...
    let result = games.filter((g) => {
      ...
      if (effectivePlatformFilter === "all") {
        // Auto: show games compatible with current device
        if (deviceReady) {
          matchPlatform =
            g.platform === "both" ||
  ...
  /* ── Tag-based sections (filtered by platform) ── */
  ...
  return (
    <div className="mx-auto max-w-7xl px-3 py-5 sm:px-4 sm:py-8">
      {/* ═══ HERO ═══ */}
      <section className="relative mb-8 sm:mb-12 overflow-hidden rounded-2xl border border-white/5 glass-card px-4 py-10 text-center sm:px-6 sm:py-20">
        {/* Animated glow blobs */}
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
          <div className="h-48 w-48 sm:h-72 sm:w-72 rounded-full bg-purple-600/20 blur-[100px] sm:blur-[140px] animate-glow-pulse" />
        </div>
        <div className="pointer-events-none absolute -top-20 -right-20 h-40 w-40 rounded-full bg-blue-600/10 blur-[80px]" />
        <div className="pointer-events-none absolute -bottom-20 -left-20 h-40 w-40 rounded-full bg-violet-600/10 blur-[80px]" />
        ...
        <div className="relative">
          ...
          <p className="mx-auto mt-3 max-w-2xl text-sm text-gray-400 sm:mt-4 sm:text-lg">
            {isMobile
              ? "Touch-optimized games for your phone. Play instantly, no downloads."
              : "Action, racing, puzzles, shooters, and casual games — all instant, all free, all in your browser."}
          </p>
      ...
      {/* ── TOP AD BANNER ── */}
      <AdBanner slot="top-banner" className="mb-6 sm:mb-8" />

      {/* ── CONTINUE PLAYING ── */}
      {continueGame && (
        <section className="mb-6 sm:mb-8 animate-fade-in-up">
          <h2 className="mb-2.5 sm:mb-3 text-base sm:text-lg font-bold text-gray-200">
            Continue Playing
          </h2>
            ...
            <div className="flex items-center gap-3 sm:gap-4 rounded-xl glass-card neon-glow-hover p-2.5 sm:p-3 transition-all active:scale-[.98]">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="mt-0.5 text-[11px] sm:text-xs text-gray-400 truncate">
                  {continueGame.category} —{" "}
                  {continueGame.description.slice(0, 50)}...
                </p>
              </div>
              <div className="shrink-0 flex h-9 w-9 sm:h-12 sm:w-12 items-center justify-center rounded-full bg-purple-600 text-white shadow-lg shadow-purple-500/30 transition-transform group-hover:scale-110">
                <svg
                  className="h-4 w-4 ml-0.5 sm:h-6 sm:w-6"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path d="M8 5v14l11-7z" />
      ...
      {trendingGames.length > 0 && (
        <section className="mb-8 sm:mb-10">
          <h2 className="mb-3 sm:mb-4 text-base sm:text-lg font-bold text-gray-200">
            Trending Games
          </h2>
      ...
      {recommendedGames.length > 0 && (
        <section className="mb-8 sm:mb-10">
          ...
          <div className="flex gap-2.5 sm:gap-3 overflow-x-auto pb-2 -mx-3 px-3 sm:-mx-0 sm:px-0 scrollbar-hide">
      ...
      {recentGames.length > 0 && (
        <section className="mb-8 sm:mb-10 animate-fade-in-up">
          <h2 className="mb-3 sm:mb-4 text-base sm:text-lg font-bold text-gray-200">
            Recently Played
          </h2>
      ...
      {favoriteGames.length > 0 && (
        <section id="favorites" className="mb-8 sm:mb-10 animate-fade-in-up">
          <h2 className="mb-3 sm:mb-4 text-base sm:text-lg font-bold text-gray-200">
            Your Favorites
          </h2>
      ...
      {/* ── MID-PAGE AD ── */}
      <AdBanner slot="mid-page" className="mb-6 sm:mb-8" />
