...
export default function InlineGamePlayer({ game, onClose }: Props) {
  ...
  return (
    ...
    >
      {/* ── Top bar ── */}
      <div
        className="flex h-11 sm:h-12 shrink-0 items-center gap-2 px-2 sm:px-4"
        style={{
      ...
      >
        ...
        {/* Title */}
        <h2 className="flex-1 truncate text-xs sm:text-sm font-semibold text-white">
          {game.title}
        </h2>

        {/* Platform tag */}
        <span className="hidden sm:block text-[10px] text-gray-500 font-medium bg-white/[0.03] px-2 py-0.5 rounded-md border border-white/[0.06]">
        ...
        <button
          ...
          className={`flex h-8 w-8 items-center justify-center rounded-lg transition-colors ${fav ? "text-red-400 bg-red-500/10" : "text-gray-500 hover:text-red-400 hover:bg-white/5"}`}
        ...
        <button
          onClick={toggleFs}
          className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-500 hover:text-white hover:bg-white/5 transition-colors"
        ...
        {failed && !loaded && (
          ...
          >
            ...
            <p className="text-xs text-gray-500 max-w-xs">
              The game server may be slow. Try again or go back.
            </p>
            <div className="flex gap-2">
              <button
                onClick={retry}
                className="btn-primary rounded-xl px-5 py-2.5 text-xs font-medium text-white"
              >
                Retry
              </button>
              <button
                onClick={onClose}
                className="rounded-xl border border-white/10 px-5 py-2.5 text-xs font-medium text-gray-300 hover:bg-white/5 active:scale-95"
              >
                Back
              </button>
            </div>
      ...
      {/* ── Bottom bar ── */}
      <div
        className="flex h-8 shrink-0 items-center px-3 sm:px-4 text-[10px] text-gray-600"
        style={{
          background: "rgba(15, 15, 26, 0.95)",
      ...
      >
        <span className="truncate">
          {game.category} &middot; {game.description.slice(0, 60)}
        </span>
        <span className="ml-auto shrink-0">
          {isMobile ? "Tap to play" : "Press ESC to close"}
        </span>
      </div>
    </div>
  );
